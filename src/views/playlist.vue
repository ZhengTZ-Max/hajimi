...
import { subscribePlaylist, deletePlaylist } from '@/api/playlist';
import { isAccountLoggedIn } from '@/utils/auth';
import nativeAlert from '@/utils/nativeAlert';
import locale from '@/locale';

import { getMockPlaylistById, mapVideoToTrack } from '@/mock/videos';
...
export default {
  ...
  methods: {
    ...
    loadData(id, next = undefined) {
      ...
      // 使用本地模拟视频数据构造歌单与轨道列表
      const mock = getMockPlaylistById(this.id);

      this.playlist = {
        id: mock.id,
        name: mock.name,
        description: mock.description,
        coverImgUrl: mock.coverUrl,
        creator: {
          userId: this.data.user.userId,
        },
        trackCount: mock.videos.length,
      };

      this.tracks = mock.videos.map(mapVideoToTrack);
      this.show = true;
